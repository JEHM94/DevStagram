...
@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2 bg-white m-5 pb-5">
            ...
            <div class="p-3 flex items-center gap-2 mt-2">
                @auth
                    @if ($post->checkLike(auth()->user()))
                        <form method="POST" action="{{ route('posts.likes.destroy', $post) }}" class="h-6">
                            @method('DELETE')
                            @csrf
                            {{-- <div class="my-4"> --}}
                            <button type="submit">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="red" viewBox="0 0 24 24" stroke-width=".5"
                    ...
                    @else
                        <form method="POST" action="{{ route('posts.likes.store', $post) }} " class="h-6">
                            @csrf
                            {{-- <div class="my-4"> --}}
                            <button type="submit">
